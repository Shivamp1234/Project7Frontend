...
const Technology = () => {
  ...
  useEffect(() => {
    axios.get("https://blog-app-backend-ptg0.onrender.com/api/category/technology")
    ...
    <>
      ...
      <div className='data-container'>
        {more ?
          <div className='data'>
            {
              data.filter((item) => { return item.category === "technology" && item.id >= 24 && item.id <= 35 }).map((element, index) => (
          ...
          <div className='data'>
            {
              data.filter((item) => { return item.category === "technology" && item.id >= 24 && item.id <= 29 }).map((element, index) => (
                <div className='data1' key={index}>
                  <Link to={`/detaildescription/${element.id}`} className='link'><img src={element.DataImage} alt=""></img></Link>
                  <div className='data-description'>
                    <Link to={`/detaildescription/${element.id}`} className='link'><h4>{element.DataTitle}</h4></Link>
